...
import { createMentor, updateMentor } from "apiSetup";
import { useRouter } from "next/router";

const EditMentorForm = ({ mentorDetails }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    const response = await updateMentor(formDatas, mentorDetails?.candidateId);
  ...
  useEffect(() => {
    // Update the form data when mentorDetails are available
    if (mentorDetails) {
      setFormData({
        ...
        userType: mentorDetails.userType || "",
        expertise: mentorDetails.expertise || [""],
        mentorOfferings: mentorDetails.mentorOfferings || [""],
        superpower: mentorDetails.superpower || "",
        profilePicture: null,
      });
    }
  }, [mentorDetails]);
